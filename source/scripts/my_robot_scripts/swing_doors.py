...
def search_drawer(
    ...
    for cabinet_pose in cabinet_poses:
        ...
        # )
        ########
        # hard code pos infront of cabinet for debugging
        body_poses = []
        offset = np.array((-0.7,2.0,0))
        coords = cabinet_pose.coordinates + offset
        body_pose = Pose3D(coordinates=coords)
        body_pose.set_rot_from_rpy((0,0,-90), degrees=True)
